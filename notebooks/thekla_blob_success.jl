...
# NEXT-100 thekla success analysis

Are the extremes in clouds / breadth
...
anas = ["clouds_mc_nsteps1", "clouds_reco_nsteps1"]
#anas = ["clouds_reco_nsteps1",]
#"nfiles249_clouds_reco_nsteps1",
#	    "nfiles249_breadth_mc_nsteps3", "nfiles249_breadth_reco_nsteps3"];
...
for afile in anas
	filename = string("bb0nu/Thekla/thekla_nodes_noconnect_nfiles249_", afile, ".csv")
	println(filename)
	dfs[string("noconnect_", afile)] = DataFrame(CSV.File(string(datadir, filename)))
end
xanas = sort(collect(keys(dfs)))
end

# ╔═╡ f064fa2d-e823-4b7c-ad87-e6b2f0c0cfa5
md"""
### Energy distribution
"""

# ╔═╡ 4a9f1470-a23a-4910-a0a5-e90d58e30112
biana = @bind iana PUI.Select(xanas)

# ╔═╡ a183f353-5b7d-4bd6-9066-a24acc85f97e
function _selection(dfe)
	sel1  = (dfe.energy  .>= 2.42) .& (dfe.energy .< 2.5)
	sel2  = sel1 .& (dfe.nclouds .== 1)
	sel3  = sel1 .& sel2 .& (dfe.nextrs .== 2)
	nevts = length(dfe.event)
	nnum  = sum(sel1), sum(sel2), sum(sel3)
	nden  = length(dfe.event), sum(sel1), sum(sel2)
	effs  = [round(n/d, digits = 3) for (n, d) in zip(nnum, nden)]
	return sel3, effs
end

# ╔═╡ ce48fdeb-5cd6-4ca9-aa3d-f0c570040190
dfs_pevt[anas[1]]
...
dfsum = Dict()
for afile in xanas
	dfsum[afile] = event_summary(dfs[afile])
end
end

# ╔═╡ 5baeeb07-2494-41b1-b04c-e022168bd33d
begin
effs     = Dict()
dfsumsel = Dict()
for ana in xanas
	sel, ieffs = _selection(dfsum[ana])
	effs[ana]  = ieffs
	dfsumsel[ana] = dfsum[ana][sel, :]
end
end

# ╔═╡ 30f70d8a-c023-4299-92d2-2999a150031c
begin

k1, k2, k3, k4 = xanas[1], xanas[2], xanas[3], xanas[4]
e1, e2, e3, e4 = effs[k1], effs[k2], effs[k3], effs[k4]	
md"""
...
| key   | Energy RoI | clouds == 1 | extremes == 2 | 
| ---:  |      --:   |         --: |         --:   |
| $(k1)  | $(e1[1]) | $(e1[2]) | $(e1[3]) |
| $(k2)  | $(e2[1]) | $(e2[2]) | $(e2[3]) |
| $(k3)  | $(e3[1]) | $(e3[2]) | $(e3[3]) |
...
# ╔═╡ 1a38bdf4-104b-4ce8-b4d6-8b1406075b58
begin
dfe = dfsumsel[iana]

ns = maximum(dfe.nnodes)
h1  = histogram(dfe.nnodes, nbins = range(0, ns, ns+1), normed = true, title = "nodes")

nbs = maximum(dfe.nnodesbs)
	
h2  = histogram(dfe.nnodesbs, nbins = range(0, nbs, nbs+1), normed = true, title = "nodes labeled as blob")

emax = maximum(dfe.eextr1)
	
h3 = histogram(dfe.eextr1, nbins = range(0., emax, 100), normed = true, title = "energy extreme 1")

h4 = histogram(dfe.eextr2, nbins = range(0., emax, 100), normed = true, title = "energy extreme 2")

h5 = histogram(dfe.enodeb1, nbins = range(0., emax, 100), normed = true, title = "energy node blob 1")

h6 = histogram(dfe.enodeb2, nbins = range(0., emax, 100), normed = true, title = "energy node blob2")

h7 = histogram(dfe.lextr1, nbins = range(0, 8, 9), normed = true, title = "label extreme 1")
	
h8 = histogram(dfe.lextr2, nbins = range(0, 8, 9), normed = true, title = "label extreme 2")

h9  = histogram(dfe.dextr1, nbins = range(0, 9, 10), normed = true, title = "dist2blob extr1")
	
h10 = histogram(dfe.dextr2, nbins = range(0, 9, 10), normed = true, title = "dist2blob extr2")
	
h11 = histogram(dfe.nextrbs, nbins = range(0, 3, 4), normed = true, title = "extremes labeled as blobs")
	
hhs = (h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11)
plot(hhs..., layout = (6, 2), size = (650, 1200), legend = false)
#h1  = histogram(dfe.nnodes, nbins = range(1:12:1)
#h1  = histogram(dfe.nnodes, nbins = range(1:12:1)
end

# ╔═╡ 9b0ce19c-8326-4119-8b77-607913d6a772
# ╠═╡ disabled = true
#=╠═╡
begin
_dfe = dfsum["clouds_reco_nsteps1"]
histogram(_dfe.energy, nbins = range(2.41, 2.50, 100))
end
  ╠═╡ =#

# ╔═╡ 5cde7013-f52e-4ad4-8f24-b88de59beaad
dfsum[anas[1]]

# ╔═╡ 6a7528d2-ba26-46c7-8255-9aeb43f399db
begin
_df = dfsum[anas[1]]
histogram(_df.energy, nbins = range(2., 2.6, 100))
end

# ╔═╡ 61b15d8c-adf8-4efa-a738-5e18b1cc0281
_df
